...
上传代码成功后，AI视觉模块会对拍到的画面进行识别，判断是否有二维码，如果有便将二维码的内容进行赋值给变量，然后通过变量进行判断是否是对应的内容，内容"Red"亮红灯，内容"Green"亮绿灯，内容"Blue"亮绿灯，内容"White"亮白灯，内容"Black"熄灭灯。识别到二维码后5秒内没有再识别到二维码便会自动灭灯。